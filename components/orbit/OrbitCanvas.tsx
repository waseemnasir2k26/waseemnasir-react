...
/* ============================================================
   ORBIT CANVAS — plain three.js, mounted imperatively in a
   useEffect (no @react-three/fiber — see SceneObjects.ts header
   for why: react-reconciler@0.27, the only line compatible with
   this repo's React 18.3.1, can't read the secret-internals shape
   this Next.js build's client bundle exposes; verified failure
...
export default function OrbitCanvas({
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(C.paper, 8, 26);
    ...
    const objects: SceneObject[] = [
      buildEntryCore(mats),
      buildStackOrbit(mats),
      buildProofField(mats),
      buildWorkIslands(mats),
      buildDockPlatform(mats),
    ];
